...
- **Install Method** — cara patch dipasang ke folder game: `method1` (PAK canonical + signature bypass), `method2` (manual loader `winhttp.dll`), `method3` (Resource Mount tanpa signature bypass). Dikenali lewat `InstallMethods.Normalize`.
- **Resource Mount** — (method3) menulis pak + sig + mount-file ke folder resource versi game yang aktif (`Client/Saved/Resources/<versi>/`), diverifikasi via owner marker `.wuwaid-resource-mount` dan konsistensi SHA-1 mount. Dibedakan dari instalasi metode lain lewat `ResourceMountInstaller.IsManaged`.
- **Resource-ready** — kondisi folder resource game yang siap dipatch method3: ada `ResManifest`, folder `Mount`, dan signature resmi (`Resource/Base/*.sig` + pak berpasangan + entri mount yang cocok SHA-1).
