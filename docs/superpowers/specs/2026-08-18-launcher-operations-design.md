...
patch notes on first open, no remote diagnostics/log-upload surface, and one
explicitly scoped anonymous active-player presence heartbeat.

## Scope and assumptions

- “Matikan fitur kirim logs” means remote diagnostics upload, crash upload, and
  user/game telemetry are unavailable until a server exists. The unused upload
  UI, IPC commands, settings fields, backend modules, and related dependency
  features are removed rather than leaving a disabled control that can fail
  later. The separately requested anonymous active-player presence heartbeat
  is not a diagnostics upload and is documented below.
...
- The existing game patch release-note feed remains the source of the
  ...
  GitHub Release in `TitoTFP/WuwaIDLauncher`, whose body combines curated
  release notes with GitHub-generated change notes. The launcher shows that
  validated entry once per launcher release tag in a separate first-open modal.
...
implementation, diagnostic heartbeat workers, and their frontend event/state
...
This leaves the launcher with no outbound log/diagnostics request path and no
setting that implies the unavailable server exists.

### 2. Active-player presence heartbeat

The focused `engine::active_player` service is the only intentional presence
request. After the existing UI-interactive milestone it sends an anonymous
`open` event, repeats a `heartbeat` every five minutes, and sends one `launch`
event after a game process is spawned. The JSON contains only `client_id`,
`launcher_version`, `install_method`, and `event`; the client ID is persisted
locally and no game path, Windows user, account data, or logs are sent.

The request is best-effort with an eight-second timeout. Network or HTTP
failures are logged locally and never become launcher UI errors, delay game
launch, or prevent shutdown. The service is cancelled on launcher exit. This
does not restore the removed logs UI or remote diagnostics path.
...
### 5. First-open launcher release notes
...
### 6. Binary-only packaging and size
...
- Active-player heartbeat failures are logged and ignored; they do not surface
  as a launcher operation error.
...
- Rust unit/integration tests cover removed IPC registration, legacy settings
  normalization, active-player payload privacy/client-ID lifecycle, launcher
  release JSON mapping/cache payloads, tray notification text, and restart
  countdown semantics.
...
- No new log server or remote diagnostics endpoint is introduced; the active
  heartbeat uses the existing presence endpoint only.
