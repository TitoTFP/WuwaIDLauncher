...
**Architecture:** Keep the existing canonical path definitions, game-path preconditions, PAK structure validation, release SHA-256 validation, staging, snapshots, and rollback. Stop writing and requiring ownership markers; cleanup always targets the canonical artifact paths and only reports `preserved` for non-file targets or real filesystem failures. Keep marker path helpers only long enough to remove markers left by older Tauri builds.
...
- [ ] **Step 1: Make validators use canonical files only**
...
- [ ] **Step 2: Stop writing markers while preserving rollback cleanup of old markers**
...
- [ ] **Step 3: Remove the foreign-target rejection gate**
...
- [ ] **Step 4: Update patch status artifact detection**
...
- [ ] **Step 1: Add the failing partial-cleanup test**
...
- [ ] **Step 2: Remove canonical files unconditionally during cleanup**
...
Use the same rule for signature bypass (`pak` plus old marker) and loader (`pak`, DLL, plus old marker). Continue restoring the signature backup and reporting actual removal errors.

- [ ] **Step 3: Update old foreign-artifact tests to the approved path model**
...
- [ ] **Step 4: Run focused cleanup and transaction tests**
...
- [ ] **Step 1: Assert new installs do not create ownership markers**
...
- [ ] **Step 2: Replace the old conflict contract**
...
- [ ] **Step 4: Run all Rust tests and frontend checks**
...
- [ ] **Step 1: Build the frontend and standalone launcher**
...
- [ ] **Step 4: Keep release tag paused**
